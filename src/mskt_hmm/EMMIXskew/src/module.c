#include "emmix.h"
#ifdef EMMIX_STANDALONE
  #include "standalone_rshim/compat.h"
  #include "standalone_rshim/special.h"
  #include "standalone_rshim/blas_lapack.h"
#endif
#include <math.h>
#include <string.h>
#include <stdlib.h>

/* -------- R bridges: compile ONLY when NOT standalone -------- */
#ifndef EMMIX_STANDALONE




/* use the R sorting helper only when R is present */
void oldmyrevsort_(double *a, int *ib, int *pn)
{
    /* Avoid R's revsort; just reuse our local sorter */
    myrevsort_(a, ib, pn);
}

/* nonzeromax_ in R build stayed here; in standalone it lives in special.c */
void nonzeromax_(double *v, int *p, double *vmax)
{
  ...
  if (m == 1) return;
  for (i = 0; i < m; ++i) {
    if (fabs((tmp = v[i])) > zero) vmax[0] = (tmp > vmax[0]) ? tmp : vmax[0];
  }
}
#endif 
...
{
    MINFO *myinfo = (MINFO*)pinfo;
    double x = dof / 2.0;
    double dg = mydigamma_(&x);  /* Fortran-callable digamma wrapper from special.c */
    return ( (log(x) - dg + 1.0) * myinfo->stau + myinfo->sxuu );
}
