#include "emmix.h"
#ifndef M_LN_SQRT_PI
#define M_LN_SQRT_PI   0.5723649429247000870717135  /* ln(sqrt(pi)) */
#endif
#ifndef M_LN_SQRT_2PI
#define M_LN_SQRT_2PI  0.9189385332046727417803297  /* ln(sqrt(2*pi)) */
#endif
...
*/
double a = (dof[k] + (double)p) / two;
double b = dof[k] / two;
double lgamma_a = mygammln_(&a);
double lgamma_b = mygammln_(&b);

lgconst = lgamma_a - (log(dof[k]) + logpi) * ((double)p)/two
        - lgamma_b - log(lgdet)/two;
